...
total.load.dt <- load.time.sys %>% sapply(., function(t){
...
```{r load.tracking.gps.df, include=FALSE, eval=knitr::is_html_output()}
ml <- 8 * 1024^3
ms <- 6 * 1024^3
...
```{r getData1.52, eval=FALSE, include=FALSE}
cities <- c(
  "Arica" = "arica",
  "Iquique" = "iquique",
  "Tocopilla" = "tocopilla",
...
get.data.gtfs.rt <- function(city = "concepcion"){
  ...
  df <- data.frame(
    ...
    start.datetime.utc = if (datetimes.utc %>% length() >= n) datetimes.utc[1:n] else rep(NA, n),
    stringsAsFactors = F
  )
  return(df)
}

real.time.data <- "concepcion" %>% get.data.gtfs.rt()
```
      ...
      routes.data <- json$routes %>% lapply(., function(route) {
        ...
        if(!(route$line %>% is.null()) && route$line %>% length() > 0) {
          route$line %>% seq_along() %>% lapply(., function(i) {
            ...
            row$point_index <- i
            row$total_points <- length(route$line)
...
total.dt <- folder.time.list %>% sapply(., function(t){
  (t %>% as.numeric()) * switch (t %>% units(),
    "secs" = 1,
    "mins" = 60,
  )
}) %>% sum()
# Reading Time: 1h + 7m + 29.16s
start.sys <- Sys.time()
...
waze.routes.df$leadAlert.position.lat <- waze.routes.df$leadAlert.position %>% sapply(., function(p){
  if(p %>% is.na()) {
    return(NA)
  }
  str_split(p, " ")[[1]][1] %>% as.numeric()
})
waze.routes.df$leadAlert.position.lng <- waze.routes.df$leadAlert.position %>% sapply(., function(p){
  if(p %>% is.na()) {
    return(NA)
  }
  str_split(p, " ")[[1]][2] %>% as.numeric()
})
...
El sistema procesó 178752 registros de datos de rutas provenientes de la API de Waze, consolidando información de navegación y alertas de tráfico durante un período de 9 días.

### Metadatos de Procesamiento

- **Tiempo de lectura**: 1 hora, 7 minutos, 29.16 segundos
- **Tiempo de unión**: 1.67 segundos
- **Tiempo de transformación**: 1 minuto, 9.94 segundos
- **Total de procesamiento**: 1 hora, 8 minutos, 40.77 segundos

### Período de Monitoreo

- **Fecha inicial**: 24 de noviembre de 2025
- **Fecha final**: 2 de diciembre de 2025
- **Duración**: 9 días completos
...
- **Registros totales**: 1025696 puntos de ruta
- **Rutas únicas (agrupadas)**: 178752 rutas
...
- "Av. Los Carrera": 114912 referencias
- "Av. Pedro Aguirre Cerda": 25536 referencias
- "Ruta 160": 12768 referencias

*Principales Orígenes* (`fromName`)

- "Av. Los Carrera": 119168 referencias
- "Av. Pedro Aguirre Cerda": 27664 referencias
- "Ruta 160": 10640 referencias

**Métricas de Desempeño de Rutas**

*Tiempos de Viaje*

- **Tiempo histórico (`historicTime`)**: 1-131 unidades (media: 20.9, mediana: 11)
- **Tiempo estimado (`time`)**: 2-148 unidades (media: 19.7, mediana: 13)
- **Unidades por confirmar**: Minutos o segundos (requiere validación)

*Distancias y Velocidades*
- **Longitud de ruta (`length`)**: 16-1257 unidades (media: 175.4, mediana: 126)
- **Velocidad calculada (`speed`)**: 0.5-24 unidades (media: 8.0, mediana: 7.8)
...
- **Total de alertas registradas**: 89

*Tipología de Alertas* (`leadAlert.type`)

**Peligros (HAZARD) - 58 casos**:

- `HAZARD_ON_ROAD`: 27 casos
- `HAZARD_ON_ROAD_OBJECT`: 22 casos
- `HAZARD_ON_ROAD_CONSTRUCTION`: 6 casos
- `HAZARD_ON_ROAD_CAR_STOPPED`: 3 casos

**Accidentes (ACCIDENT) - 31 casos**:

- `NO_SUBTYPE`: 26 casos
- `ACCIDENT_MAJOR`: 5 casos

*Interacción de Usuarios con Alertas*

- **Comentarios** (`numComments`): 0-37 (media: 7.9, mediana: 4)
- **Me gusta** (`numThumbsUp`): 0-37 (media: 7.5, mediana: 4)
- **Reportes de inexistencia** (`numNotThereReports`): 0-1

*Contexto Geográfico de Alertas*

- **Calles más reportadas**:
  - "Ruta 160": 63 alertas
  - "Av. Pedro Aguirre Cerda": 25 alertas
  - "Av. Los Carrera Poniente": 1 alerta
- **Ciudades involucradas**:
  - "San Pedro de la Paz": 25 alertas
  - "Concepción": 1 alerta
  - Sin especificar: 63 alertas
