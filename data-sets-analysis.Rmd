...
manual.offset.curve <- function(geometry, offset){
  coords <- geometry %>% st_coordinates()
  x <- coords[, "X"]
  y <- coords[, "Y"]
  if(coords %>% nrow() < 2) return(geometry[[1]])
  dx <- x %>% diff()
  dy <- y %>% diff()
  l <- (dx ^ 2 + dy ^ 2) %>% sqrt()
  l[l == 0] <- 1e-8
  ...
  if(nx %>% length() == 1){
    ...
    nx.avg <- c(nx[1], (nx[-1] + nx[-(nx %>% length())]) / 2, nx[nx %>% length()])
    ny.avg <- c(ny[1], (ny[-1] + ny[-(ny %>% length())]) / 2, ny[ny %>% length()])
  }
  l.avg <- (nx.avg ^ 2 + ny.avg ^ 2) %>% sqrt()
  l.avg[l.avg == 0] <- 1e-8
  nx.avg <- nx.avg / l.avg
  ny.avg <- ny.avg / l.avg
  x.new <- x + offset * nx.avg
  y.new <- y + offset * ny.avg
  cbind(x.new, y.new) %>% st_linestring()
}
...
get.street <- function(
  ...
  message("1. Getting Local Data...")
  sf::sf_use_s2(F)
  bbox.poly <- points.wgs %>% st_buffer(dist = 0.005) %>% st_bbox() %>% st_as_sfc()
  streets <- local.streets.map[bbox.poly,] %>% suppressWarnings()
  sf::sf_use_s2(T)
  if(streets %>% nrow() == 0) stop("No Streets Found.")
  points.m <- points.wgs %>% st_transform(crs = 3857)
  streets.m <- streets %>% st_transform(crs = 3857) %>% suppressWarnings(st_cast("LINESTRING"))
  message("2. Processing Streets Directions...")
  oneway <- streets.m$oneway %in% c("F", "T")
  oneway.streets <- streets.m[oneway,]
  two.ways.streets <- streets.m[!oneway,]
  message("3. Calculating Bidirectional Lanes...")
  if(two.ways.streets %>% nrow() > 0){
    original.geoms <- two.ways.streets %>% st_geometry()
    reverse.geoms <- original.geoms %>% st_reverse()
    go.sfc <- original.geoms %>% future_map(.f = ~manual.offset.curve(.x, offset = offset), .progress = T) %>% st_sfc(crs = 3857)
    back.sfc <- reverse.geoms %>% future_map(.f = ~manual.offset.curve(.x, offset = offset), .progress = T) %>% st_sfc(crs = 3857)
    go <- two.ways.streets
    st_geometry(go) <- go.sfc
    back <- two.ways.streets
    st_geometry(back) <- back.sfc
    double.streets <- rbind(go, back)
    final.streets <- rbind(oneway.streets, double.streets)
  } else {
    final.streets <- oneway.streets
  }
  final.streets$lane.id <- 1:(final.streets %>% nrow())
  message("4. Calculating Distancies to the Points...")
  near.indexes <- st_nearest_feature(points.m, final.streets)
  near.lanes <- final.streets[near.indexes,]
  geom.points <- points.m %>% st_geometry()
  geom.streets <- near.lanes %>% st_geometry()
  short.lines.list <- purrr::map2(geom.points, geom.streets, ~st_nearest_points(.x, .y)[[1]])
  short.lines <- short.lines.list %>% st_sfc(crs = 3857)
  all.points <- short.lines %>% st_cast(to = "POINT")
  points.on.street.m <- all.points[seq(2, all.points %>% length(), by = 2)]
  final.points.wgs <- points.on.street.m %>% st_transform(crs = 4326)
  final.coords <- final.points.wgs %>% st_coordinates()
  results <- points.df %>% mutate(
    lng.street = final.coords[, "X"],
    lat.street = final.coords[, "Y"],
    street.name = ifelse(near.lanes$name %>% is.na(), NA, near.lanes$name),
    lane.id = near.lanes$lane.id,
    distance = st_distance(points.m, near.lanes, by_element = T) %>% as.numeric()
  )
  results$street.geom <- near.lanes %>% st_geometry() %>% st_transform(crs = 4326)
...
quantum.data <- function(
  ...
  if(is.point.data){
    ...
    dt$lng.street <- centered.points$lng.street
    dt$lat.street <- centered.points$lat.street
    dt$street.name <- centered.points$street.name
    dt$lane.id <- centered.points$lane.id
    dt$street.geom <- centered.points$street.geom
    dt <- dt[!is.na(lng.street) & !is.na(lat.street)]
    dt.f <- dt %>% as.data.table()
    setorder(dt.f, name, id)
    sf.points <- dt.f %>%
      st_as_sf(coords = c("lng.street", "lat.street"), crs = 4326) %>%
      st_transform(crs = 3857)
    coords <- sf.points %>% st_coordinates()
    dt.f$X <- coords[, "X"]
    dt.f$Y <- coords[, "Y"]
    dt.f$geom.street.3857 <- sf.points$street.geom %>% st_geometry() %>% st_transform(3857)
    dt.f[, next.X := X %>% shift(type = "lead"), by = name]
    dt.f[, next.Y := Y %>% shift(type = "lead"), by = name]
    dt.f[, next.street := street.name %>% shift(type = "lead"), by = name]
    dt.f[, next.lane := lane.id %>% shift(type = "lead"), by = name]
    dt.f[, next.idx := .I %>% shift(type = "lead"), by = name]
    dt.lines <- dt.f[!is.na(next.X)]
    dt.lines <- dt.lines[(X != next.X) | (Y != next.Y)]
    if(dt.lines %>% nrow() == 0) stop("No Lines Generated.")
    dt.lines$next.geom.street <- dt.f$geom.street.3857[dt.lines$next.idx]
    dt.same <- dt.lines[lane.id == next.lane | lane.id %>% is.na() | next.lane %>% is.na()]
    dt.same[, final.X := next.X]
    dt.same[, final.Y := next.Y]
    dt.same[, seg.lane := lane.id]
    dt.same[, seg.street := street.name]
    dt.diff <- dt.lines[lane.id != next.lane & !is.na(lane.id) & !is.na(next.lane)]
    if(dt.diff %>% nrow() > 0){
      int.pts <- purrr::map2(dt.diff$geom.street.3857, dt.diff$next.geom.street, function(g1, g2) {
        res <- st_intersection(g1, g2) %>% suppressWarnings()
        if(res %>% length() == 0 || res %>% st_is_empty()) return(c(NA, NA))
        if(inherits(res, "MULTIPOINT")) res <- st_cast(res, "POINT")[[1]]
        if(inherits(res, "POINT")) return(st_coordinates(res)[1, c("X", "Y")])
        return(c(NA, NA))
      })
      int.mat <- do.call(rbind, int.pts)
      dt.diff$int.X <- int.mat[, 1]
      dt.diff$int.Y <- int.mat[, 2]
      dt.diff.valid <- dt.diff[!is.na(int.X)]
      dt.diff.invalid <- dt.diff[int.X %>% is.na()]
      if(dt.diff.invalid %>% nrow() > 0){
        dt.diff.invalid[, final.X := next.X]
        dt.diff.invalid[, final.Y := next.Y]
        dt.diff.invalid[, seg.lane := lane.id]
        dt.diff.invalid[, seg.street := street.name]
      }
      if(dt.diff.valid %>% nrow() > 0){
        part1 <- dt.diff.valid %>% copy()
        part1[, final.X := int.X]
        part1[, final.Y := int.Y]
        part1[, seg.lane := lane.id]
        part1[, seg.street := street.name]
        
        part2 <- dt.diff.valid %>% copy()
        part2[, X := int.X]
        part2[, Y := int.Y]
        part2[, final.X := next.X]
        part2[, final.Y := next.Y]
        part2[, seg.lane := next.lane]
        part2[, seg.street := next.street]
        part2[, geom.street.3857 := next.geom.street]
        dt.diff.valid.expanded <- rbind(part1, part2)
      } else {
        dt.diff.valid.expanded <- data.table()
      }
      dt.segments <- rbind(
        dt.same[, .(id, name, time.bin, field, X, Y, final.X, final.Y, seg.lane, seg.street, geom.street.3857)], 
        dt.diff.invalid[, .(id, name, time.bin, field, X, Y, final.X, final.Y, seg.lane, seg.street, geom.street.3857)],
        dt.diff.valid.expanded[, .(id, name, time.bin, field, X, Y, final.X, final.Y, seg.lane, seg.street, geom.street.3857)],
        fill = T)
    } else {
      dt.segments <- dt.same[, .(id, name, time.bin, field, X, Y, final.X, final.Y, seg.lane, seg.street, geom.street.3857)]
    }
    list.lines <- lapply(1:(dt.segments %>% nrow()), function(i) {
      c(dt.segments$X[i], dt.segments$final.X[i], dt.segments$Y[i], dt.segments$final.Y[i]) %>% matrix(ncol = 2) %>% st_linestring()
    })
    dt.segments$geometry <- list.lines %>% st_sfc(crs = 3857)
    sf.data <- dt.segments %>% st_sf()
    ...
    sf.data <- st_intersection(sf.data, grid.sf) %>% 
      st_collection_extract(type = "LINE") %>% 
      st_cast(to = "LINESTRING", warn = F)
    grid.polys <- st_geometry(grid.sf)[sf.data$space.bin]
    exact.geom.list <- purrr::map2(sf.data$geom.street.3857, grid.polys, function(line, poly) {
      res <- st_intersection(line, poly) %>% suppressWarnings()
      if(res %>% length() == 0) return(NULL)
      res.sfc <- res %>% st_sfc(crs = 3857)
      res.lines <- res.sfc %>% st_collection_extract("LINE") %>% suppressWarnings()
      if(length(res.lines) == 0) return(NULL)
      res.cast <- res.lines %>% st_cast("LINESTRING") %>% suppressWarnings()
      return(res.cast[[1]])
    })
    valid.idx <- !sapply(exact.geom.list, is.null)
    sf.data <- sf.data[valid.idx, ]
    sf.data$geometry <- exact.geom.list[valid.idx] %>% st_sfc(crs = 3857)
    sf.data$weight <- sf.data %>% st_length() %>% as.numeric()
    dt.f <- sf.data %>% as.data.table()
    group.cols <- intersect(c("name", "time.bin", "space.bin", "seg.street"), dt.f %>% names())
    group.cols <- intersect(c("name", "time.bin", "space.bin", "seg.lane", "seg.street"), dt.f %>% names())
    compiled.data <- dt.f[, {
      c.field <- compile.function(X = field, W = weight)
      geom.comb <- geometry %>% st_combine()
      new.geom <- (geom.comb %>% st_union() %>% st_line_merge())[[1]]
      .(compiled.field = c.field, angle.rad = NA, geometry = list(new.geom))
    }, by = group.cols]
    setnames(compiled.data, "seg.street", "street.name")
    setnames(compiled.data, "seg.lane", "lane.id")
...
server <- function(input, output, options){
  ...
  sd <- filtered.waze.routes %>% SharedData$new()
  f.data <- reactive({
    input$time.range %>% req()
    discretization.example %>%
      filter(time.bin >= input$time.range[1] & time.bin <= input$time.range[2]) %>%
      group_by(geometry, name) %>%
      summarise(
        field = compiled.field %>% mean(),
        geometry = geometry %>% first(),
        time.info = paste(time.bin %>% min() %>% format(., "%H:%M"), "-", time.bin %>% max() %>% format(., "%H:%M")),
        date.info =  ifelse(time.bin %>% min() %>% format(., "%Y-%m-%d") == time.bin %>% max() %>% format(., "%Y-%m-%d"), time.bin %>% min() %>% format(., "%Y-%m-%d"), paste(time.bin %>% min() %>% format(., "%Y-%m-%d"), "-", time.bin %>% max() %>% format(., "%Y-%m-%d"))),
        time.bin = time.bin %>% range(na.rm = T) %>% mean(na.rm = T),
        space.bin = space.bin %>% first(),
        name = name %>% first(),
        .groups = 'drop'
      )
  })
...
line.to.polygon <- function(geometry, thickness = 2) {
  math.polygon <- function(coords, radius) {
    x <- coords[, "X"]
    y <- coords[, "Y"]
    if(x %>% length() < 2) return(st_polygon())
    dx <- x %>% diff()
    dy <- y %>% diff()
    l <- sqrt(dx^2 + dy^2)
    l[l == 0] <- 1e-8
    ...
    if(nx %>% length() == 1) {
      ...
      nx.avg <- c(nx[1], (nx[-1] + nx[-length(nx)]) / 2, nx[length(nx)])
      ny.avg <- c(ny[1], (ny[-1] + ny[-length(ny)]) / 2, ny[length(ny)])
    }
    l.avg <- sqrt(nx.avg ^ 2 + ny.avg ^ 2)
    l.avg[l.avg == 0] <- 1e-8
    nx.avg <- nx.avg / l.avg
    ny.avg <- ny.avg / l.avg
    x.right <- x - radius * nx.avg
    y.right <- y - radius * ny.avg
    x.left <- rev(x + radius * nx.avg)
    y.left <- rev(y + radius * ny.avg)
    
    poly.coords <- cbind(
      c(x.right, x.left, x.right[1]),
      c(y.right, y.left, y.right[1])
    )
    return(list(poly.coords) %>% st_polygon())
  }
  is.sfg <- inherits(geometry, "sfg")
  if (is.sfg) geometry <- geometry %>% st_sfc(crs = 4326)
  orig.crs <- geometry %>% st_crs()
  if (orig.crs %>% is.na()) {
    orig.crs <- st_crs(4326)
    geometry <- geometry %>% st_set_crs(4326)
  }
  geom.m <- geometry %>% st_transform(crs = 3857)
  poly.list <- geom.m %>% st_geometry() %>% lapply(function(g) {
    if (inherits(g, "LINESTRING")) {
      return(math.polygon(g %>% st_coordinates(), thickness / 2))
    } else if (inherits(g, "MULTILINESTRING")) {
      parts <- g %>% st_sfc() %>% st_cast(to = "LINESTRING") %>% lapply(function(part) {
        math.polygon(part %>% st_coordinates(), thickness / 2)
      })
      return(parts %>% lapply(unclass) %>% st_multipolygon())
    } else if (inherits(g, "GEOMETRYCOLLECTION")) {
      parts <- g %>% st_collection_extract("LINESTRING") %>% st_sfc() %>% st_cast(to = "LINESTRING") %>% lapply(function(part) {
        math.polygon(part %>% st_coordinates(), thickness / 2)
      })
      if(length(parts) > 0) return(parts %>% lapply(unclass) %>% st_multipolygon())
      else return(st_polygon())
    } else {
      return(st_polygon())
    }
  })
  poly.sfc <- poly.list %>% st_sfc(crs = 3857)
  poly.final <- poly.sfc %>% st_transform(crs = orig.crs)
  if (is.sfg) return(poly.final[[1]])
  return(poly.final)
}

get.gps.street <- get.street(
  lng.data = gps.data.df$Longitud,
  lat.data = gps.data.df$Latitud
)

discretization.example.1 <- quantum.data(
  time.data = gps.data.df$Fecha,
  lng.data = gps.data.df$Longitud,
  lat.data = gps.data.df$Latitud,
  field.data = gps.data.df$`Velocidad Km/h`,
  names.data = gps.data.df$Vehiculo,
  street.aligned = T,
  time.gap = tg.1,
  space.gap = sg.1
)

discretization.polygon <- discretization.example.1 %>% mutate(
  geometry = geometry %>% line.to.polygon(thickness = 50)
)

base.grid <- discretization.example.1 %>%
  as.data.frame() %>%
  select(street.name, lane.id, space.bin, geometry) %>%
  filter(!is.na(lane.id)) %>%
  distinct(lane.id, space.bin, .keep_all = T) %>%
  st_as_sf()
...
server.1 <- function(input, output, options){
  ...
  f.data <- reactive({
    input$time.range %>% req()
    (input$selected.vehicles %>% length() > 0) %>% req()
    real.data <- discretization.example.1 %>%
      filter(
        time.bin >= input$time.range[1] & 
        time.bin <= input$time.range[2] &
        name %in% input$selected.vehicles
      ) %>%
      group_by(space.bin, lane.id, street.name) %>%
      summarise(
        field = compiled.field %>% mean(na.rm = T),
        geometry = geometry %>% first(),
    ...
    unnamed.data <- real.data %>% filter(lane.id %>% is.na())
    named.real <- real.data %>% filter(!is.na(lane.id))
    active.lanes <- named.real$lane.id %>% unique()
    if(active.lanes %>% length() > 0) {
      interpolated.data <- base.grid %>%
        filter(lane.id %in% active.lanes) %>%
        arrange(lane.id, space.bin) %>%
        left_join(named.real %>% as.data.frame() %>% select(-geometry), by = c("lane.id", "space.bin", "street.name")) %>%
        group_by(lane.id) %>%
        mutate(
          field = {
            valid <- !is.na(field)
            n.valid <- valid %>% sum()
            if (n.valid >= 2) {
              approx(x = space.bin[valid], y = field[valid], xout = space.bin, rule = 2)$y 
            } else if (n.valid == 1) {
              rep(field[valid], n())
            } else {
              field
          ...
          vehicles.involved = ifelse(vehicles.involved %>% is.na() & !is.na(field), "Interpolado", vehicles.involved),
          time.info = ifelse(time.info %>% is.na() & !is.na(field), "Interpolado", time.info),
          date.info = ifelse(date.info %>% is.na() & !is.na(field), "Interpolado", date.info)
        ) %>%
        filter(!is.na(field)) %>%
        ungroup() %>%
        st_as_sf()
      interpolated.data <- interpolated.data %>%
        arrange(lane.id, space.bin) %>%
        group_by(lane.id) %>%
        mutate(next.geom = geometry %>% lead()) %>%
        ungroup()
        
      bridged.geoms <- purrr::map2(interpolated.data$geometry, interpolated.data$next.geom, function(g1, g2) {
        if(g2 %>% is.null() || g2 %>% length() == 0) return(g1)
        c1 <- g1 %>% st_coordinates()
        c2 <- g2 %>% st_coordinates()
        if(c1 %>% nrow() == 0 || c2 %>% nrow() == 0) return(g1)
        p1 <- c1[nrow(c1), c("X", "Y")]
        p2 <- c2[1, c("X", "Y")]
        dist <- sqrt((p1["X"] - p2["X"])^2 + (p1["Y"] - p2["Y"])^2)
        if(dist > 0.00005 && dist < 0.03) {
          bridge <- rbind(p1, p2) %>% st_linestring()
          return(st_union(g1, bridge))
        } else {
          return(g1)
        }
      }) %>% st_sfc(crs = 4326)
      interpolated.data$geometry <- bridged.geoms
      interpolated.data <- interpolated.data %>% select(-next.geom) %>% st_sf()
      final.data <- bind_rows(interpolated.data, unnamed.data) %>% st_as_sf()
    } else {
      final.data <- unnamed.data
    }
    final.data <- final.data %>% mutate(
      geometry = geometry %>% line.to.polygon(thickness = 20)
    )
    return(final.data)
  })
  output$map <- renderLeaflet({
      ...
      proxy %>%
        addPolygons(
          ...
          popup = ~paste(
            ...
            paste("Calle:", ifelse(is.na(street.name), "Sin Nombre", street.name)),
            paste("ID Carril:", lane.id),
            paste("Fecha:", date.info),
            paste("Hora:", time.info),
