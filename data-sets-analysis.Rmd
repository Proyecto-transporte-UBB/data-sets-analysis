...
meters.per.degree.lng <- function(lat.d) meters.per.degree.lat * cos(lat.d * pi / 180)
...
quantum.data <- function(
  ...
  if(is.point.data){
    ...
    if(street.aligned & !(names.data %>% is.na() %>% all())){
      sf.lines <- sf.data %>%
        group_by(name) %>%
        summarise(geometry = geometry %>% st_combine() %>% st_cast("LINESTRING"))
    }
    grid <- sf.data %>% st_make_grid(cellsize = space.gap * c(1, street.angle.tolerance %>% sin()))
    sf.data$space.bin <- st_intersects(sf.data, grid) %>% as.integer()
    sf.data$weight <- 1
    ...
    sf.data$len <- sf.data %>% st_length()
    grid <- sf.data %>% st_make_grid(cellsize = space.gap * c(1, street.angle.tolerance %>% sin()))
    grid.sf <- st_sf(
      space.bin = 1:(grid %>% length()),
      geometry = grid
    )
    ...
    if(street.aligned){
      get.angle <- function(geom){
        pts <- geom %>% st_coordinates()
        if(pts %>% nrow() < 2) return(NA)
        p1 <- pts[1,]
        p2 <- pts[pts %>% nrow(),]
        atan2(p2["Y"] - p1["Y"], p2["X"] - p1["X"])
      }
      sf.data$angle.rad <- sf.data %>% st_geometry() %>% sapply(FUN = get.angle)
    }
  } else {
    stop("You must provide 'lng.data' and 'lat.data', or 'geom.data'.")
  }
  dt.f <- sf.data %>% as.data.table()
  group.cols <- c("name", "time.bin", "space.bin") %>% intersect(., dt.f %>% names())
  compiled.data <- dt.f[, {
    c.field <- field %>% compile.function(W = weight)
    a.rad <- if(street.aligned & "angle.rad" %in% (.SD %>% names())) angle.rad %>% mean(na.rm = T) else NA
    coords.c <- geometry %>% st_combine() %>% st_centroid() %>% st_coordinates()
    xc <- coords.c[1, "X"]
    yc <- coords.c[1, "Y"]
    ang <- if(a.rad %>% is.na()) 0 else a.rad
    dx <- ang %>% cos() * (space.gap / 2)
    dy <- ang %>% sin() * (space.gap / 2)
    new.geom <- c(xc - dx, xc + dx, yc - dy, yc + dy) %>% matrix(ncol = 2) %>% st_linestring()
    .(
      compiled.field = c.field,
      angle.rad = a.rad,
      geometry = list(new.geom)
    )
  }, by = group.cols]
...
quantum.data <- function(
  ...
  dt.f <- sf.data %>% as.data.table()
  group.cols <- intersect(c("name", "time.bin", "space.bin"), names(dt.f))
  
  compiled.data <- dt.f[, {
    c.field <- compile.function(X = field, W = weight)
    a.rad <- if(street.aligned && "angle.rad" %in% names(.SD)) mean(angle.rad, na.rm = TRUE) else NA
    
    geom_sfc <- if (inherits(geometry, "sfc")) geometry else do.call(st_sfc, c(geometry, list(crs = 3857)))
    geom_combined <- st_combine(geom_sfc)
    
    if (is.point.data) {
      coords <- st_coordinates(geom_combined)
      if (nrow(coords) > 1) {
        # Castea los MULTIPOINT a LINESTRING
        new.geom <- st_cast(geom_combined, "LINESTRING")[[1]]
      } else if (nrow(coords) == 1) {
        # Si hay un único punto en este bin, duplicamos su coordenada 
        # para forzar la creación de un LINESTRING estricto (de longitud cero)
        pt <- c(coords[1, "X"], coords[1, "Y"])
        new.geom <- st_linestring(matrix(c(pt, pt), ncol = 2, byrow = TRUE))
      } else {
        new.geom <- st_linestring()
      }
    } else {
      new.geom <- st_line_merge(st_union(geom_combined))[[1]] 
    }
  ...
  compiled.data$name <- as.factor(compiled.data$name)
  compiled.data$geometry <- st_sfc(compiled.data$geometry, crs = 3857)
  return(compiled.data %>% st_sf() %>% st_transform(crs = 4326))
}
```
...
server <- function(input, output, options){
  ...
  f.data <- reactive({
    ...
    discretization.example %>%
      filter(time.bin >= input$time_range[1] & time.bin <= input$time_range[2]) %>%
...
server.1 <- function(input, output, options){
  ...
  sd <- discretization.example.1 %>% SharedData$new()
  f.data <- reactive({
    input$time_range %>% req()
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      addLayersControl(
        overlayGroups = discretization.example.1$name %>% levels(),
        options = layersControlOptions(collapsed = F)
      )
  })
  observe({
