...
```{r load.tracking.gps.df, include=FALSE, eval=FALSE}
ml <- 8 * 1024^3
ms <- 6 * 1024^3
...
ggplot(gh026929) +
  geom_histogram(aes(x = `GPS (2D speed) [m/s]`))
```


```{r gh0269291, echo=FALSE, warning=FALSE}
ggplot(gh026929) +
  geom_histogram(aes(x = precision))
```


```{r gh0269292.html, echo=FALSE, warning=FALSE, eval=knitr::is_html_output()}
pal <- colorNumeric(
...
```{r gh0269292, echo=FALSE, warning=FALSE, eval=!knitr::is_html_output()}
ggplot() +
...
nearest.camera <- st_nearest_feature(
  radio.accidents.df.sf,
  biobio.inventary.sf
)
distances <- st_distance(
  radio.accidents.df.sf,
  biobio.inventary.sf[nearest.camera, ],
  by_element = T
)
radio.accidents.df.camera.distance <- radio.accidents.df
radio.accidents.df.camera.distance$Cámara <- biobio.inventary$ID[nearest.camera]
radio.accidents.df.camera.distance$`Distancia a Cámara` <- as.numeric(distances)
...
### Distribución por distancia a la cámara más cercana

```{r radio.accidents.df3, echo=FALSE, warning=FALSE}
peak.radio <- radio.accidents.df.camera.distance[radio.accidents.df.camera.distance$Hora >= "2024-08-01 20:00" & radio.accidents.df.camera.distance$Hora <= "2024-08-01 22:00",]
ggplot(radio.accidents.df.camera.distance) +
  geom_density(aes(x = `Distancia a Cámara`, colour = `Tipo de incidente` %>%
      fct_infreq() %>%
      fct_rev())) +
  labs(colour = "Tipo de incidente") +
  scale_x_log10()
```
...
gps.events.chile.camera.distance <- gps.events.chile
gps.events.chile.camera.distance$Cámara <- biobio.inventary$ID[nearest.camera]
gps.events.chile.camera.distance$`Distancia a Cámara` <- as.numeric(distances)
ggplot() +
  annotation_map_tile(type = "osm", zoom = 17) +
  geom_point(data = gps.events.chile.camera.distance, aes(x = longitud_, y = latitud_, colour = evento_, size = `Distancia a Cámara`)) +
  labs(x = "Longitud", y = "Latitud") +
  coord_sf(crs = 4326)
```

```{r gps.events1.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Argentina**")
```
...
```{r biobio.inventary2.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Concepción**")
```
...
```{r biobio.inventary3.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**San Pedro de la Paz**")
```
...
```{r biobio.inventary4.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Chiguayante**")
```
...
```{r biobio.inventary5.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Talcahuano**")
```
...
trafic.alerts.camera.distance <- trafic.alerts
trafic.alerts.camera.distance$Cámara <- biobio.inventary$ID[nearest.camera]
trafic.alerts.camera.distance$`Distancia a Cámara` <- as.numeric(distances)
...
```{r trafic.alerts2.title0, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Concepción, San Pedro de la Paz, Talcahuano, Hualpén, Penco y Hualqui**")
```

```{r trafic.alerts2.title1, echo=FALSE, eval=!knitr::is_html_output()}
cat("*JAM*")
```
...
```{r trafic.alerts3.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*HAZARD*")
```
...
```{r trafic.alerts4.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*WEATHERHAZARD*")
```
...
```{r trafic.alerts5.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ACCIDENT*")
```
...
```{r trafic.alerts6.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ROAD_CLOSED*")
```
...
```{r trafic.alerts7.title0, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Los Ángeles**")
```

```{r trafic.alerts7.title1, echo=FALSE, eval=!knitr::is_html_output()}
cat("*JAM*")
```
...
```{r trafic.alerts8.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*HAZARD*")
```
...
```{r trafic.alerts9.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*WEATHERHAZARD*")
```
...
```{r trafic.alerts10.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ACCIDENT*")
```
...
```{r trafic.alerts11.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ROAD_CLOSED*")
```
...
```{r trafic.alerts12.title0, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Chiguayante**")
```

```{r trafic.alerts12.title1, echo=FALSE, eval=!knitr::is_html_output()}
cat("*JAM*")
```
...
```{r trafic.alerts13.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*HAZARD*")
```
...
```{r trafic.alerts14.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*WEATHERHAZARD*")
```
...
```{r trafic.alerts15.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ACCIDENT*")
```
...
```{r trafic.alerts16.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ROAD_CLOSED*")
```
...
```{r trafic.alerts17.title0, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Tomé**")
```

```{r trafic.alerts17.title1, echo=FALSE, eval=!knitr::is_html_output()}
cat("*JAM*")
```
...
```{r trafic.alerts18.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*HAZARD*")
```
...
```{r trafic.alerts19.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*WEATHERHAZARD*")
```
...
```{r trafic.alerts20.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ACCIDENT*")
```
...
```{r trafic.alerts21.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("*ROAD_CLOSED*")
```
...
```{r accidents1.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Concepción, San Pedro de la Paz, Talcahuano, Hualpén, Penco, Hualqui, Lirquén y Lenga**")
```
...
```{r accidents2.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Los Ángeles**")
```
...
```{r accidents3.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Chiguayante**")
```
...
```{r accidents4.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Tomé**")
```
...
```{r accidents5.title, echo=FALSE, eval=!knitr::is_html_output()}
cat("**Huertos Familiares**")
```
