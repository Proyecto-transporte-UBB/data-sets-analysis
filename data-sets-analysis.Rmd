...
interurban.bus.accidents %>% mutate(
  Fecha = Fecha %>% as.numeric(),
  Región = `Cód-Región`,
  Comuna = Comuna %>% as.numeric(),
  `Tipo (CONASET)` = `Tipo (CONASET)` %>% as.numeric(),
  Zona = Zona %>% as.numeric(),
  `Causa (CONASET)` = `Causa (CONASET)` %>% as.numeric(),
  Ruta = Ruta %>% as.numeric(),
  `Ubicación Relativa` = `Ubicación Relativa` %>% as.numeric()
) %>% select(-Año, -`Cód-Región`, -`Tipo Accidente`, -`Causa Accidente`, -`Calle Uno`, -`Calle Dos`, -Intersección) %>% cor() %>% ggcorrplot() +
