...
quantum.data <- function(
  ...
  dt.expanded <- dt[, {
    cxm <- lat %>% meters.per.degree.lng() * min(lng, mX, na.rm = T)
    cym <- min(lat, mY, na.rm = T) * meters.per.degree.lat
  ...
  result <- dt.expanded[, {
    ...
    list(
      ...
      lng = (minX %>% min() + maxX %>% max()) / 2,
      lat = (minY %>% min() + maxY %>% max()) / 2,
      minX = minX %>% min(),
      maxX = maxX %>% max(),
      minY = minY %>% min(),
      maxY = maxY %>% max(),
  ...
  }, by = .(time.bin, x.bin, y.bin, heading.bin)]
...
server <- function(input, output, options){
  ...
  pal.dir <- colorNumeric(
    palette = c("#F00", "#FF0", "#0F0", "#0FF", "#00F", "#F0F", "#F00"),
    domain = c(0, 360)
  )
  ...
  f.data <- reactive({
    ...
    plot.data.sf %>%
      ...
      summarise(
        ...
        time.bin = time.bin %>% middle.value(),
        x.bin = x.bin %>% first(),
        y.bin = y.bin %>% first(),
        heading.bin = heading.bin %>% first(),
        .groups = 'drop'
      )
  })
  abs.f.data <- reactive({
  ...
  observe({
    ...
    proxy %>%
      addPolygons(
        data = data.to.plot,
        fillColor = ~field %>% pal(),
        color = ~street.heading.from.north %>% pal.dir(),
        fillOpacity = 0.8,
        weight = 0.8,
        popup = ~paste(
          ...
          paste("Longitud:", lng %>% degrees.format()),
          paste("Latitud:", lat %>% degrees.format()),
          paste0("Dirección: ", street.heading.from.north %>% round(), "°"),
          paste0("Bin: (", paste(x.bin, y.bin, heading.bin, sep = ", "), ")"),
          sep = "<br>"
        )
      ) %>%
      addLegend(
