...
quantum.data <- function(
  ...
  dt[, lat.m := meters.per.degree.lat * lat]
  dt[, lng.m := lat %>% meters.per.degree.lng() * lng]
  dt[, x.along := heading %>% cos() * lng.m + heading %>% sin() * lat.m]
  dt[, y.across := -heading %>% sin() * lng.m + heading %>% cos() * lat.m]
  dt.expanded <- dt[, {
    bx <- (x.along / space.gap) %>% floor()
    by <- (y.across / y.gap.meters) %>% floor()
    .(
      time.bin = time.bin,
      x.bin = bx,
      y.bin = by,
      minX.bin = bx * space.gap,
      maxX.bin = (bx + 1) * space.gap,
      minY.bin = by * y.gap.meters,
      maxY.bin = (by + 1) * y.gap.meters,
      speed = speed,
      name = name,
      lng = lng,
      lat = lat,
  ...
  result <- dt.expanded[, {
    list(
      time = (time.bin * time.gap + first.date) %>% middle.value(),
      lng = lng %>% middle.value(),
      lat = lat %>% middle.value(),
      minX = minX.bin[1],
      maxX = maxX.bin[1],
      minY = minY.bin[1],
      maxY = maxY.bin[1],
      ...
      summarised = name[1],
      n.points = .N,
      street.heading = heading %>% median(na.rm = T),
      street.heading.from.north = 90 - heading %>% median(na.rm = T) * 180 / pi + ifelse(heading %>% median(na.rm = T) > pi / 2, 360, 0)
    )
...
[Ver Diagrama de Flujo](https://mermaid.live/edit#pako:eNptVFFu2zgQvQpBIEALKFrbUWJHQFtk3TbNzyJI0p_aQTGhxgqxFKmSVFFvkMPsAfZrD7DA5mI7JGVb9tYftjSeeW_emyGfuDAV8pLXFtpHdvd-qRl9bj1Y_2pxpaWQpmTfOtC-a_IKPNy_ZsfHb9k1WGjc4gaFfJCWtWBf_mzQW-NK5mWDGVO6pi_wGVtJVFUWw3kNbcZcC2Lz6C2iz0HJWiMlCdO0UmG-6rTw0uj71FD6TqSR_47AfpV6MQclOgU2oT9IXbKVMia9s1-2pHs4fXEEunIXifvp33_2m2Fv3rC7m88f3j0Pa4-O2A00wKhKI1TAgFEPCl36ewtH4Oz25a_I8YkSpa53zVbSohDy5W_NKmQi1G-SWOdAV4ZSVmhRCwmOCdTeAnGwNaMR6EmvZlMSOOZGf_8IwhvrdjyrGKC6wEIJaF0kTZOi1mnsVRjZ_CtNitDpV9c9-gAxMtyGqe15TnY7hmGaMnbXWrNG4QMkQ32gUkEvdM1IjHahGVA91wY7El1a07W9jYuL2na0Xaw1uxlngfmrMrqWviMHQBFoCP0feH9ov5nt3H4yLvp3qPNWNq3CA7VRiA-0RLnpICn1xh2oSQg7TT3izyURbF6HrACs6_R8qIJm4oySFURbU3hoV-S6qGuLNXhiapUUxHR4qhg5JqBpTdBDp5rmFZoRYZ9Dc-Z-AD1QsUUetrUNJvc8-MEG0ko0WEkTnfMSiTPbOJjtDFyH9fQY-2k7HRrSeWuk9m7PgogeeW7Qdcov5hbDeaJTka_ockC6bMI6GFuhpj3sPQ60PU6qixAfdPVq8VHq-9fL3krXPaSLcMmvrRHooJF09gy7RFPTDbeSwix5yh0cwF1gcGZ2wc06pAjqimd038qKl952mPEGbQPhlT-FlCX3j9jgkpf0WIH9PTA-U00L-osxzaaMZlM_8nIFytFb15IJ-F4CCWi2UbpBKrRzQ5bycnJ-EkF4-cR_8HJ8Os2Ls6I4nRVnk8lsNp1mfM3L46I4z0cn5yeTs8l4dDYeFafPGf8jEk_y8_FoWsyK2Xh6Oipms5Pn_wDIyRlm)

```{mermaid}
graph TD
    Start([Inicio: quantum.data]) --> Params[Recibir parámetros: time, lng, lat, field, time.gap, space.gap, street.aligned, compile.function]
    
    Params --> TimeBin[Calcular time.bin: floor time / time.gap]
    
    TimeBin --> IsAligned{¿street.aligned == TRUE?}
    
    %% Rama Alineada a Calles
    IsAligned -- Sí --> Heading[Calcular dirección de calle Heading usando diferencias centrales y atan2]
    Heading --> ConvFactors[Calcular factores de conversión metros a grados: C_lat y C_lng]
    ConvFactors --> SpaceBin[Calcular bins espaciales proyectados en dirección de la calle y transversal]
    SpaceBin --> GroupAligned[Agrupar por time.bin, bin_longitudinal y bin_transversal]
    
    %% Rama No Alineada
    IsAligned -- No --> SpaceBinSimple[Calcular bins de latitud y longitud directos]
    SpaceBinSimple --> GroupSimple[Agrupar por time.bin, lat.group y lng.group]
    
    %% Consolidación
    GroupAligned --> Aggregate[Aplicar compile.function al campo de datos por cada grupo]
    GroupSimple --> Aggregate
    
    Aggregate --> Stats[Calcular promedios de tiempo, latitud, longitud y conteo de puntos n.points]
    
    Stats --> Result[Crear data.frame final ordenado por tiempo]
    Result --> End([Fin])

    subgraph "Procesamiento Geográfico"
        Heading
        ConvFactors
        SpaceBin
    end
...
discretization.example <- quantum.data(
  ...
  field.data = filtered.waze.routes$id,names.data = filtered.waze.routes$name,
...
server <- function(input, output, options){
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
      addTiles() %>%
      addLayersControl(
        overlayGroups = c("Límites Originales", "Velocidad Discretizada"),
  ...
  observe({
    ...
    proxy %>% 
      addPolygons(
        data = abs.data.to.plot,
        color = "black",
        fill = F,
        group = "Límites Originales"
      ) %>%
      addPolygons(
        data = data.to.plot,
        fillColor = ~pal(field),
        ...
        popup = ~paste0(
          ...
          "<br>Dirección: ", street.heading.from.north, "°"),
        group = "Velocidad Discretizada"
      )
  })
}
