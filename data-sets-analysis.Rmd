...
key.objects <- c("interurban.bus.accidents", "gh026929", "police.accidents", "radio.accidents.df", "chiguayante.speed.df", "vega.speed.df", "gps.events", "biobio.inventary", "trafic.alerts", "accidents", "waze.data", "meters.per.degree.lat", "meters.per.degree.lng", "quantum.data", "tracking.gps.df", "tracking.gps.df.cor", "waze.routes", "waze.routes.df")
...
$$
\Theta = \left\{ x \in X \mid f\left( x \right) \notin R\left( f\left( X \right) \right) \right\}
$$

- $\Theta\subseteq \mathbb{R}$: Conjunto de outliers
- $X\subseteq \mathbb{R}$: Conjunto de datos
- $f: \mathbb{R}\to \mathbb{R}$: Función de escala (puede ser $f\left( x \right) = x$ o $f\left( x \right) = \ln \left( x \right)$)
- $R: \mathcal{P}\left( \mathbb{R} \right)\to \mathcal{P}\left( \mathbb{R} \right)$: Función de intervalo

## Función de intervalo

### Método de media y desviación standard

$$
R\left( K \right) = \left[ \text{mean}\left( K \right) \pm n \times \text{sd}\left( K \right) \right]
$$

- $\text{mean}: \mathcal{P}\left( \mathbb{R} \right)\to \mathbb{R}$: Media aritmética
- $\text{sd}: \mathcal{P}\left( \mathbb{R} \right)\to \mathbb{R}$: Desviación standard
- $n \in \mathbb{R}$: Diferencia con la media en términos de desviaciones standard (por defecto: $n = 2$)

### Método de cuartiles

$$
R\left( K \right) = \left[ Q_{1}\left( K \right) - n \times \text{IQ}\left( K \right), Q_{3}\left( K \right) + n \times \text{IQ}\left( K \right) \right]
$$

- $Q_{k}: \mathcal{P}\left( \mathbb{R} \right)\to \mathbb{R}$: k-ésimo cuartil
- $\text{IQ}: \mathcal{P}\left( \mathbb{R} \right)\to \mathbb{R}$: Rango intercuartílico ($\text{IQ}\left( K \right) = Q_{3}\left( K \right) - Q_{1}\left( K \right)$)
- $n \in \mathbb{R}$: Diferencia con los cuartiles en términos de rangos intercuartílicos (por defecto: $n = \frac{3}{2}$)
...
tracking.gps.df.cities <- st_join(tracking.gps.df.points, cities.map, join = st_intersects)
...
- Diferencia Media: 5 minutos, 33 segundos (lineal); 5 minutos, 3.66 segundos (logarítmico)
- Diferencia Mediana: 5 minutos
- Desviación Standard: 30 minutos, 34.06 segundos (lineal); 1.18 segundos (logarítmico)
...
Hay 39 outliers inferiores y 147 outliers superiores, dando un total de 186 outliers
...
Hay 91270 outliers inferiores y 36508 outliers superiores, dando un total de 127778 outliers
