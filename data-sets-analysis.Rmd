...
- Diferencia Media: 14 horas, 7 minutos y 6 segundos
- Diferencia Mediana: 8 horas y 25 minutos
- Desviación Standard: 18 horas y 56.73 segundos
...
interurban.bus.accidents.dates <- data.frame(
  ...
  Mes = interurban.bus.accidents$Fecha %>% month()
)
...
interurban.bus.accidents.causes <- data.frame(
  `Causa (CONASET)` = interurban.bus.accidents.n.top.causes$`Causa (CONASET)`,
...
interurban.bus.accidents.causes %>%
  ...
  ggplot() +
  geom_tile(aes(y = Causa..CONASET., x = Región, fill = n)) +
  scale_fill_gradientn(
    name = "Cantidad",
    colors = c(
      "red",
...
- Desviación Standard: 2 horas, 24 minutos, 17.9 segundos
...
positive.diff <- differences[differences > 0]
#positive.diff %>% summary()
diff.mean <- positive.diff %>% mean(na.rm = T)
diff.median <- positive.diff %>% median(na.rm = T)
ggplot() +
  geom_histogram(aes(x = positive.diff)) +
  geom_vline(aes(xintercept = diff.mean, colour = "promedio")) +
  geom_vline(aes(xintercept = diff.median, colour = "mediana")) +
  scale_x_log10() +
  scale_color_manual(
...
tracking.gps.df.dates <- data.frame(
  ...
  `Día de la Semana` = tracking.gps.df$EVT_GPS_TIME_CHILE_STR %>% weekdays()
)
...
- Distancia Media: 334 metros
- Distancia Mediana: 240.1 metros
- Desviación Standard: 919.54 metros
...
ggplot() +
  ...
  geom_vline(aes(xintercept = tracking.gps.df.distances %>% mean(na.rm = T), colour = "promedio")) +
  geom_vline(aes(xintercept = tracking.gps.df.distances %>% median(na.rm = T), colour = "mediana")) +
  scale_x_log10() +
  scale_y_log10() +
  scale_color_manual(
    values = c("promedio" = "red", "mediana" = "green"),
...
police.accidents.dates <- data.frame(
  ...
  `Día de la Semana` = police.accidents$FECHA %>% weekdays()
)
tracking.gps.df.dates %>%
  count(Hora, Día.de.la.Semana, .drop = F) %>%
  ggplot() +
...
- Diferencia Media: 5 horas, 44 minutos, 36 segundos
- Diferencia Mediana: 1 hora, 5 minutos, 30 segundos
- Desviación Standard: 20 horas, 32 minutos, 52.93 segundos
...
radio.accidents.df.dates <- data.frame(
  ...
  `Día de la Semana` = radio.accidents.df$Hora %>% weekdays()
)
...
- Distancia Media: 733.6 metros
- Distancia Mediana: 141.1 metros
- Desviación Standard: 2.49 kilómetros
...
- Diferencia Media: 35 minutos, 55 segundos
- Diferencia Mediana: 6 minutos
- Desviación Standard: 1 hora, 4 minutos, 2.76 segundos
...
- Diferencia Media: 3 minutos, 40.5 segundos
- Diferencia Mediana: 2 minutos
- Desviación Standard: 13 minutos, 10.19 segundos
...
#positive.differences %>% summary()
ggplot() +
  geom_density(aes(x = positive.differences)) +
  scale_x_log10()
```
...
- Diferencia Media: 15.03 segundos
- Diferencia Mediana: 1 segundo
- Desviación Standard: 18.82 segundos
...
#differences[differences > 0] %>% summary()
ggplot() +
  geom_histogram(aes(x = differences[differences > 0])) +
  labs(x = "differences")
```
...
- Distancia Media: 65.5 kilómetros
- Distancia Mediana: 4.59 metros
- Desviación Standard: 168 kilómetros
...
- Distancia Media: 1.95 kilómetros
- Distancia Mediana: 519.8 metros
- Desviación Standard: 9.56 kilómetros
...
```{r trafic.alerts.time, echo=FALSE, message=FALSE, warning=FALSE}
...
accidents.dates <- data.frame(
  Mes = accidents$Date %>% month(),
  `Día de la Semana` = accidents$Date %>% weekdays()
)
accidents.dates %>%
...
- Distancia Media: 3.27 metros
- Distancia Mediana: 55.36 centímetros
- Desviación Standard: 37.17 metros
...
waze.data.dates <- data.frame(
  Mes = waze.data$Day %>% month(),
  `Día de la Semana` = waze.data$Day %>% weekdays()
)
waze.data.dates %>%
  count(Mes, Día.de.la.Semana, .drop = F) %>%
  ggplot() +
...
waze.routes.dates <- data.frame(
  ...
  `Día de la Semana` = waze.routes$updateTime %>% weekdays()
)
waze.routes.dates %>%
  count(Hora, Día.de.la.Semana, .drop = F) %>%
  ggplot() +
...
- Distancia Media: 53.59 metros
- Distancia Mediana: 29.31 metros
- Desviación Standard: 76.16 metros

```{r waze.routes.df.space, echo=FALSE, warning=FALSE}
...
```{r waze.routes.length, echo=FALSE, message=FALSE, warning=FALSE}
waze.routes %>% ggplot() +
  stat_density_2d(
