...
quantum.data <- function(
  ...
  if(is.point.data){
    ...
    lanes.unique <- sf.data %>%
      as.data.frame() %>%
      select(seg.lane, geom.street.3857) %>%
      filter(!is.na(seg.lane)) %>%
      distinct(seg.lane, .keep_all = T) %>%
      #st_as_sf(crs = 3857) %>%
      mutate(l = geom.street.3857 %>% st_length() %>% as.numeric())
    grid.sf <- lanes.unique %>%
      mutate(cuts = purrr::map(l, ~{
        if(.x <= space.gap){
          data.frame(start.f = 0, end.f = 1)
        } else {
          fracs <- seq(0, 1, by = space.gap / .x)
          if(fracs[fracs %>% length()] != 1) fracs <- c(fracs, 1)
          data.frame(start.f = fracs[-length(fracs)], end.f = fracs[-1])
        }
      })) %>%
      unnest(cuts) %>%
      mutate(
        geometry = purrr::pmap(
          list(geom.street.3857, start.f, end.f),
          function(geom, s, e){
            if(s == 0 & e == 1) return(geom)
            lwgeom::st_linesubstring(geom, s, e)
          }
        ) %>% st_sfc(crs = 3857),
        space.bin = row_number()
      ) %>%
      rename(seg.lane.grid = seg.lane) %>%
      select(space.bin, seg.lane.grid, geometry) %>%
      st_as_sf(crs = 3857)
    grid.polys.sf <- grid.sf %>% 
      st_buffer(dist = 15, endCapStyle = "FLAT", joinStyle = "MITRE")
    sf.data <- st_intersection(sf.data, grid.polys.sf) %>% 
      filter(seg.lane == seg.lane.grid | is.na(seg.lane)) %>% 
      st_collection_extract(type = "LINE") %>% 
      st_cast(to = "LINESTRING", warn = F)
    grid.polys <- (grid.polys.sf %>% st_geometry())[match(sf.data$space.bin, grid.polys.sf$space.bin)]
...
line.to.polygon <- function(line, thickness = 2) {
  #if(!inherits(line, c("sf, sfc"))) stop("The input 'line' must be an sf or sfc object (LINESTRING or MULTILINESTRING)")
...
server.1 <- function(input, output, options){
  ...
  f.points.centered <- reactive({
    ...
    gps.streets %>% mutate(
      Fecha = gps.data.df$Fecha,
      Vehiculo = gps.data.df$Vehiculo
    ) %>% filter(
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      setView(
        ...
        overlayGroups = c("Calles", "Puntos Originales", "Puntos Centrados", "Celdas")
      )
  })
  observe({
    proxy <- leafletProxy("map") %>% clearShapes()
    ...
    if(updated.data %>% nrow() > 0){
      updated.polygons <- updated.data %>% 
        mutate(geometry = geometry %>% line.to.polygon(thickness = 5)) %>% 
        st_as_sf()
      proxy %>%
        addPolylines(
          data = updated.data,
          ...
          popup = ~paste(
            ...
            paste("Calle:", ifelse(is.na(street.name), NA, street.name)),
