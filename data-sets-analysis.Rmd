...
display.heat.map = function(placeName = "Av. Los Carrera", dirName = "O.P", filter = T) {
  ...
  plot.data %>% 
    ...
    labs(title = paste("Mapa de calor en", placeName, "en dirección", dirName), x = "Tramo", y = "Hora", fill = "Velocidad") +
...
for(placeName in waze.routes.heatmap$placeName %>% levels()){
  for(dirName in waze.routes.heatmap$dirName %>% levels()){
    display.heat.map(placeName, dirName, waze.routes.heatmap$date >= "2025-12-03") %>% print()
  }
}
```
