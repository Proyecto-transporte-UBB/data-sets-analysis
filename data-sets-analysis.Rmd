...
waze.routes.df$section.dir <- waze.routes.df$name %>% str_split_i(., "_", 2) %>% as.factor()
waze.routes.df$section.name <- waze.routes.df$name %>% str_split_i(., "_", 3) %>% as.factor()
waze.routes.df$section.num <- waze.routes.df$name %>% str_split_i(., "_", 4) %>% as.numeric()
...
waze.routes <- waze.routes.df[!(waze.routes.df$point_index %>% is.na()),] %>%
  group_by(id, date, file) %>%
  mutate(
    firstX = x[point_index %>% which.min()],
    lastX  = x[point_index %>% which.max()],
    firstY = y[point_index %>% which.min()],
    lastY  = y[point_index %>% which.max()]
  ) %>%
  ungroup() %>%
  distinct(id, date, file, .keep_all = T) %>%
  select(-x, -y, -point_index)
waze.routes %>% summary()
waze.routes.df.short <- waze.routes.df %>%
  rename(
    ...
    date_str = date,
    sect_num = section.num,
    sect_dir = section.dir,
...
create.bbox.polygons <- function(df, deletePoints = T){
  polygons.list <- list()
  ...
  df.sf.0 <- df.valid %>% st_sf(geometry = polygons.list[valid.rows] %>% st_sfc(crs = 4326))
  df.sf <- if(deletePoints) df.sf.0 %>% select(-minX, -minY, -maxX, -maxY) else df.sf.0
  return(df.sf)
}
...
lines.geometry <- waze.routes.df %>%
  distinct(name, point_index, .keep_all = T) %>%
  filter(!(x %>% is.na()), !(y %>% is.na())) %>%
  st_as_sf(
    coords = c("x", "y"),
    crs = 4326
  ...
  group_by(name) %>%
  summarise(
    geometry = geometry %>% st_combine(),
    .groups = "drop"
  ) %>%
  st_cast("LINESTRING")
create.lines <- function(df) df %>%
  inner_join(
    lines.geometry,
    by = "name"
  ) %>%
  st_as_sf()
waze.routes.lines <- waze.routes %>% create.lines()
...
chile.streets <- st_read("~/GitHub/data-sets-analysis/shapefiles/Chile/gis_osm_roads_free_1.shp")
chile.streets$name[chile.streets$name == "Avenida Los Carrera Poniente"] <- "Avenida Los Carrera"
chile.streets$osm_id <- chile.streets$osm_id %>% as.numeric()
chile.streets$fclass <- chile.streets$fclass %>% as.factor()
chile.streets$name <- chile.streets$name %>% as.factor()
chile.streets$ref <- chile.streets$ref %>% as.factor()
chile.streets$oneway <- chile.streets$oneway %>% as.factor()
chile.streets$bridge <- chile.streets$bridge %>% as.factor()
chile.streets$tunnel <- chile.streets$tunnel %>% as.factor()

get.street <- function(
  lng.data = NA,
  lat.data = NA,
  local.streets.map = chile.streets
){
  points.df <- data.frame(lng = lng.data, lat = lat.data)
  points.wgs <- points.df %>% st_as_sf(coords = c("lng", "lat"), crs = 4326)
  message("1. Searching for Local Streets...")
  sf::sf_use_s2(F)
  bbox.poly <- points.wgs %>% st_buffer(dist = 0.005) %>% st_bbox() %>% st_as_sfc()
  streets.local <- local.streets.map[bbox.poly,] %>% suppressWarnings()
  sf::sf_use_s2(T)
  if(streets.local %>% nrow() == 0) stop("No Streets Found.")
  message("2. Consolidating Entire Streets...")
  streets.m <- streets.local %>%
    st_transform(crs = 3857)
  streets.merged <- streets.m %>%
    group_by(name) %>%
    summarise(
      geometry = geometry %>% st_combine(),
      .groups = "drop"
    ) %>%
    mutate(geometry = geometry %>% geos::as_geos_geometry() %>% geos::geos_line_merge() %>% st_as_sfc()) %>%
    st_set_crs(3857) %>%
    suppressWarnings(st_cast("MULTILINESTRING")) %>%
    suppressWarnings(st_cast("LINESTRING")) %>%
    mutate(lane.id = paste(name, row_number(), sep = "_"))
  points.m <- points.wgs %>% st_transform(crs = 3857)
  message("3. Matching attributes...")
  near.indexes <- st_nearest_feature(points.m, streets.merged)
  unique.geos.lines <- streets.merged %>% geos::as_geos_geometry()
  geos.lines <- unique.geos.lines[near.indexes]
  geos.pts <- points.m %>% geos::as_geos_geometry()
  dist.along <- geos::geos_project(geos.lines, geos.pts)
  snapped.geos <- geos::geos_interpolate(geos.lines, dist.along)
  snapped.coords <- snapped.geos %>% 
    st_as_sfc() %>% 
    st_set_crs(3857) %>% 
    st_transform(crs = 4326) %>% 
    st_coordinates()
  streets.merged.4326 <- streets.merged %>% st_transform(4326)
  results <- points.df %>%
    mutate(
      lng.street = snapped.coords[, "X"],
      lat.street = snapped.coords[, "Y"],
      street.name = streets.merged$name[near.indexes] %>% as.factor(),
      lane.id = streets.merged$lane.id[near.indexes],
      dist.along.lane = dist.along
    )
  results$street.geom <- streets.merged.4326$geometry[near.indexes]
...
quantum.data <- function(
  ...
  time.gap = 5,
  space.gap = 50,
  compile.function = function(X, W) X %>% weighted.mean(w = W, na.rm = T)
) {
  n.rows <- max(time.data %>% length(), lng.data %>% length(), geom.data %>% length(), na.rm = T)
  dt <- data.table(
    id = 1:n.rows,
    field = if(field.data %>% is.na() %>% all()) rep(NA, n.rows) else field.data,
    name = if(names.data %>% is.na() %>% all()) rep(NA, n.rows) else names.data
  )
  if(!(time.data %>% is.na() %>% all())){
    dt$time.bin <- (time.data %>% as.numeric() %/% time.gap * time.gap) %>% as.POSIXct(origin = "1970-01-01")
  } else {
    dt$time.bin <- NA
  }
  is.point.data <- !(lng.data %>% is.na() %>% all()) & !(lat.data %>% is.na() %>% all())
  is.line.data <- !(geom.data %>% is.na() %>% all())
  if(is.point.data){
    centered.points <- get.street(
      lng.data = lng.data,
      lat.data = lat.data
    )
    big.line <- centered.points %>%
      ungroup() %>%
      st_drop_geometry() %>%
      st_as_sf(sf_column_name = "street.geom") %>%
      distinct(street.name, lane.id, .keep_all = T) %>%
      select(street.name, lane.id) %>%
      st_collection_extract("LINE") %>%
      st_cast("MULTILINESTRING", warn = F) %>%
      st_cast("LINESTRING", warn = F) %>%
      mutate(fragment.id = paste(lane.id, row_number(), sep = "_"))
    small.pieces <- big.line %>%
      st_transform(crs = 3857) %>%
      mutate(
        len = street.geom %>% st_length() %>% as.numeric(),
        cuts = len %>% purrr::map(.f = function(l){
          if(l <= space.gap) return(
            data.frame(
              start.frac = 0,
              end.frac = 1
            )
          )
          dists <- seq(0, l, by = space.gap)
          if(dists[dists %>% length()] < l) dists <- c(dists, l)
          fracs <- dists / l
          data.frame(
            start.frac = fracs[-length(fracs)],
            end.frac = fracs[-1]
          )
        })
      ) %>%
      tidyr::unnest(cuts) %>%
      mutate(
        small.geom = list(street.geom, start.frac, end.frac) %>% purrr::pmap(.f = function(g, s, e) lwgeom::st_linesubstring(g, s, e)) %>% st_sfc(crs = 3857)
      ) %>%
      st_drop_geometry() %>%
      st_as_sf(sf_column_name = "small.geom") %>%
      st_transform(crs = 4326) %>%
      mutate(piece.id = paste(fragment.id, row_number(), sep = "_")) %>%
      select(-len, -start.frac, -end.frac)
    centered.points$field <- dt$field
    centered.points$name <- dt$name
    centered.points$time.bin <- dt$time.bin
    centered.points$weight <- 1
    pts.sf <- centered.points %>%
      st_as_sf(coords = c("lng.street", "lat.street"), crs = 4326) %>%
      st_transform(crs = 3857)
    sp.3857 <- small.pieces %>% st_transform(crs = 3857)
    near.idx <- st_nearest_feature(pts.sf, sp.3857)
    pts.sf$space.bin <- sp.3857$piece.id[near.idx]
    geom.list <- st_geometry(sp.3857)
    names(geom.list) <- sp.3857$piece.id
    dt.p <- pts.sf %>% as.data.table()
    group.cols <- intersect(c("name", "time.bin", "space.bin"), dt.p %>% names())
    compiled.data <- dt.p[, {
      c.field <- compile.function(X = field, W = weight)
      piece.geom <- geom.list[[.BY$space.bin]]
      .(compiled.field = c.field, geometry = list(piece.geom))
    }, by = group.cols]
  } else if (is.line.data){
    sf.data <- dt %>% st_sf(geometry = geom.data)
    if(sf.data %>% st_crs() %>% is.na()) st_crs(sf.data) <- 4326
    sf.data <- sf.data %>% st_transform(crs = 3857)
    grid <- sf.data %>% st_make_grid(cellsize = space.gap * c(1, street.angle.tolerance %>% sin()))
    grid.sf <- st_sf(space.bin = 1:(grid %>% length()), geometry = grid)
    sf.data <- st_intersection(sf.data, grid.sf) %>% st_collection_extract(type = "LINE") %>% st_cast(to = "LINESTRING", warn = F)
    sf.data$weight <- sf.data %>% st_length() %>% as.numeric()
    dt.f <- sf.data %>% as.data.table()
    group.cols <- intersect(c("name", "time.bin", "space.bin"), dt.f %>% names())
    compiled.data <- dt.f[, {
      c.field <- compile.function(X = field, W = weight)
      geom.comb <- st_combine(geometry)
      new.geom <- (geom.comb %>% st_union() %>% st_line_merge())[[1]]
      .(compiled.field = c.field, geometry = list(new.geom))
    }, by = group.cols]
  } else {
    stop("You must provide 'lng.data' and 'lat.data' or 'geom.data'.")
  }
  compiled.data <- compiled.data[!sapply(compiled.data$geometry, is.null)]
  compiled.data$name <- compiled.data$name %>% as.factor()
  compiled.data$geometry <- compiled.data$geometry %>% st_sfc(crs = 3857)
  compiled.data$lenght <- compiled.data$geometry %>% st_length()
  return(compiled.data %>% st_sf() %>% st_transform(crs = 4326))
}
```

## Diagrama de Flujo

[Ver Diagrama de Flujo](https://mermaid.live/edit#pako:eNq1Vk1v4zYQ_SuEDs0u4DU2_kpWaLdInWRRwPUamxQF6hgBLY1lNhSpUtQ22SA_pscectpbLwXqP9ZHUrLlINue6kNEajjvzQzfjHIfJTqlKI4yw4s1uzy9Ugy_C8uNfTH_XolE6Jj9WnFlq7ybcssXL9mrV28ZTPYU2_nYEDfMWbqWLyWxJS-Jfb00b5lIO2wlSOKheE6LgN14ephLkdN4TcnN_d9_nd2K0tauFu893bcPwWt7EG7sYvPovcdcJs4wd4tKIg7vtxQqoFQlV6kOLzNeLJ6BmmqPNK2k9EhbgG_Y9KR2aGj8SRf65V0REIK9cf6Sfe8VEr0UhWZpnSmMumyy3DvpwptVCuZwUqrMl4R9xSS3YekYZ1ooOzM6obL8Asxk86iI1zgZ6XznPRGK_t15KlRWqVCnM2O0eTG_uHw_i8MmYJ5zabliidYmJcVTcGlPRNZsHiGagF1WyyC0wMhzQcjP1SIkGk65XzupeUa2W1pDZGM2hofBTRetypBCRVjOJf5-FFwyqRMuFz7i70TmUpyf3cKNDJN1KXJOgMRi8ydU3tQGkUis4S0JD8-FZBa7wGo8j33hKGeCEOR8rA2apvFERKUzdgtvbYGbTLN88_utyDUrC560lelbb4fpOX7gNln7apTz0l4jdoPAr1fEbWUoZielyBSvryFBsKEyDFXw3F2ROj6XjEm40i2qFnRIh-xPJLK1ndn5Wem7GU51lK4n7thv3o7mOGyHvPMLbanzQkgCyklmqsJdFnTiRkBn26CdFqwP_Y4VBnJJhSuidlLyIN1VpRIrtGrxbfE92zuozFHVdYAEdrpjyI0nUHpdf9m-FVciF19TElLpf6m07qNdKK3uqSehXv5COFyufBahcaGoRnZIEsXAVPVNsqiv-IbeGZG6-82xvs6wgdI9HtZO1uTK5aTtETFj3U07lTUSQgG5ylAwqyUZrhLau-nAUI9uS6akxDaconmBqEAbhCzbI8Ol4vsCwxwPL-Q6shbLHvC-oibqeUWVlKH_Am6jg6QZ5luxIUZJKrPr50Q3UW3Rgeh_FV3N5kQHKgRWKZzDSHYxQgzXOZkMbRmmYlW6i34qSgsJyM3nDGMnh7Loqf6CokNWkrhyE3lnafLdt2w1ey4Ul-ITxLL5rFrhN8fnZ1LkwgVlKMMH1-jWHbeCnP44mQR5nmuTc3uOPtIGY059JGOFYQeutgeMs2Bplart0Mhg_OFijka5gzpAzRn2rH88PArcuC7M780f1vhRXBhKRLk3dgPE9qs_8XLYffcTLatccXbgdLK2BwGWO2AXay3YXXa4tZVw0_Jp7Xfgnut9ZYsK_wl9QE8btdfeK1fo7ffHBTfo90aLl1Encv0bxdZU1Imgh5y7bXTvGK4iu6acrqIYy5Sbm6voSj3Ap-DqZ63zxs3oKltH8YrLEruqwPeaTgXHHefbtwZRkxlrzPsoPhqNPEgU30e3UTzod4-Hh6PR60F_eNg_6vU60V0U97qDN296w_7gcNQbHA8fOtEnzznovu73hoeD0dEQhuFg8PAPEj9poQ)

```{mermaid}
graph TD
    Start([Inicio: quantum.data]) --> InitData[Crear data.table base <br> id, field, name]
    InitData --> TimeCheck{¿Existe <br> time.data?}
    TimeCheck -- Sí --> CalcTime[Calcular time.bin <br> usando time.gap]
    TimeCheck -- No --> NullTime[time.bin = NA]
    CalcTime --> DataTypeCheck
    NullTime --> DataTypeCheck
    DataTypeCheck{¿Tipo de <br> Datos?}
    DataTypeCheck -- Puntos <br> lng.data & lat.data --> PointProcess
    DataTypeCheck -- Líneas <br> geom.data --> LineProcess
    DataTypeCheck -- Ninguno --> Error([STOP: Error <br> Faltan coordenadas o geometría])
    subgraph Procesamiento de Puntos
        PointProcess[get.street: Centrar puntos <br> en la malla vial local] --> BigLine[Extraer líneas maestras únicas <br> de las calles centradas]
        BigLine --> SmallPieces[Cortar calles en small.pieces <br> de largo máximo space.gap]
        SmallPieces --> MatchPoints[st_nearest_feature: Asignar <br> cada punto al piece.id más cercano]
        MatchPoints --> SetWeightPt[Establecer space.bin y weight = 1]
        SetWeightPt --> CompilePt[Agrupar por name, time.bin, space.bin <br> y promediar con compile.function]
        CompilePt --> GeomPt[Asignar la geometría intacta <br> del small.piece al grupo]
    end
    subgraph Procesamiento de Líneas
        LineProcess[Crear objeto sf con <br> las líneas proporcionadas] --> MakeGrid[st_make_grid: Crear grilla espacial <br> basada en space.gap y angle.tolerance]
        MakeGrid --> IntersectGrid[st_intersection: Cortar líneas <br> con las celdas de la grilla]
        IntersectGrid --> SetWeightLn[Establecer space.bin según celda <br> y calcular weight = st_length]
        SetWeightLn --> CompileLn[Agrupar por name, time.bin, space.bin <br> y promediar con compile.function]
        CompileLn --> GeomLn[st_union & st_line_merge: <br> Fusionar la geometría topológicamente]
    end
    GeomPt --> CleanData
    GeomLn --> CleanData
    subgraph Finalización
        CleanData[Eliminar registros <br> con geometría NULL] --> FormatFactor[Convertir 'name' a Factor]
        FormatFactor --> SetCRS[Proyectar a CRS 3857 <br> para métricas precisas]
        SetCRS --> CalcLength[Calcular columna 'lenght' <br> a partir de la geometría unificada]
    end
    CalcLength --> Output([Retornar objeto sf final <br> en CRS 4326])
```
...
El algoritmo transforma un conjunto continuo de trayectorias y eventos en una matriz discreta espaciotemporal. Sea un conjunto de observaciones $O = \{o_1, o_2, ..., o_n\}$, donde cada observación $o_i$ está definida por una tupla $(t_i, p_i, v_i, e_i)$ que representa el tiempo, la posición espacial, la variable de interés (campo) y la entidad emisora (ej. vehículo). 

El objetivo es mapear cada observación a un "bin" espacio-temporal discreto $B_{j,k}$.

### 1. Discretización Temporal

El dominio del tiempo se particiona en intervalos regulares de tamaño $\Delta t$ (`time.gap`). A cada observación se le asigna un índice temporal $T_j$ mediante la función piso:

$$T_j = \left\lfloor \frac{t_i}{\Delta t} \right\rfloor \cdot \Delta t$$

### 2. Discretización Espacial

La partición del espacio depende de la topología geométrica de entrada:

- **Para datos puntuales (coordenadas):** Sea $S$ la red vial continua. Esta red se fragmenta en sub-segmentos $s_k$ tales que la longitud topológica de cada segmento cumple $L(s_k) \le \Delta s$ (`space.gap`). Cada punto $p_i$ se proyecta ortogonalmente a la coordenada más cercana en la red vial $\hat{p}_i \in S$, y luego se le asigna el contenedor espacial $s_k$ que minimiza la distancia espacial.
- **Para datos lineales (geometrías):** Se superpone una malla regular bidimensional (Grid) $G$ sobre el área de estudio. Las dimensiones de cada celda $C_k \in G$ están escaladas por $\Delta s$ y deformadas verticalmente por un factor trigonométrico dependiente de la tolerancia angular $\theta$ (`street.angle.tolerance`): $C_{alto} = \Delta s \cdot \sin(\theta)$. Las líneas continuas se intersecan matemáticamente con los polígonos de $G$, dividiéndose en sub-segmentos $l_{i,k} = p_i \cap C_k$.

### 3. Consolidación (Compilación)

Una vez agrupadas las observaciones en los subconjuntos discretos $B_{j,k}$, se aplica una función matemática de agregación $F$ (por defecto, la media ponderada) sobre los valores $v_i$.

$$V_{j,k} = \frac{\sum_{o_i \in B_{j,k}} v_i \cdot w_i}{\sum_{o_i \in B_{j,k}} w_i}$$

Donde $w_i$ es el peso matricial (para puntos $w_i = 1$; para líneas $w_i = L(l_{i,k})$).

## Parámetros de Entrada

- **`time.data`**: Vector con las marcas de tiempo de las observaciones (formato numérico o `POSIXct`).
- **`lng.data` / `lat.data`**: Vectores numéricos con las coordenadas espaciales (longitud y latitud) en EPSG:4326. Obligatorio si se procesan datos de puntos.
- **`geom.data`**: Vector de objetos geométricos (clase `sfc`). Obligatorio si se procesan datos espaciales pre-construidos (líneas o trayectorias).
- **`field.data`**: Vector numérico que contiene la variable principal de estudio que será promediada o agrupada (ej. velocidad, aceleración, flujo).
- **`names.data`**: Vector de caracteres o factores que identifica a los agentes que generaron los datos (ej. placas de vehículos, IDs de usuarios).
- **`time.gap`**: Tamaño del intervalo temporal $\Delta t$ para la agrupación (escalado en la unidad subyacente de `time.data`, usualmente segundos).
- **`space.gap`**: Tamaño objetivo $\Delta s$ (en metros) para la segmentación de la geometría espacial. Dicta el largo máximo de los segmentos discretos.
- **`compile.function`**: Expresión o función personalizada que dicta cómo se agregan matemáticamente los datos dentro de un "bin". Por defecto, utiliza `weighted.mean(X, w = W, na.rm = T)`.

## Proceso de Discretización

La arquitectura del procesamiento opera mediante un flujo de trabajo bifurcado según la naturaleza geométrica de la información ingresada.

### Fase 1: Inicialización y Temporalidad

1. Se construye una estructura tabular base unificando campos de identificación, variables numéricas y el identificador de tiempo.
2. Se normalizan todos los valores de tiempo ajustándolos a ventanas discretas cerradas definidas por el divisor `time.gap`.

### Fase 2: Bifurcación Espacial

El motor evalúa las variables declaradas y decide el método de extracción espacial:

- **Ruta A: Procesamiento de Puntos (Map Matching & Substring)**
  1.  **Alineación (Snapping):** Llama a la sub-función `get.street` que busca la malla vial de OpenStreetMap local, consolida los trazos, y proyecta las coordenadas originales hacia el eje de la calle más cercana.
  2.  **Extracción Maestra:** Obtiene las líneas únicas de las calles consolidadas y descarta duplicados, limpiando superposiciones originadas por múltiples registros en un mismo lugar.
  3.  **Corte Geométrico (Substringing):** Utiliza los largos totales proyectados (`st_length`) y los divide por el `space.gap` para obtener fracciones de inicio y fin ($s, e$). Luego aplica `st_linesubstring` para cortar la malla real en fragmentos exactos (`small.pieces`).
  4.  **Asignación (Nearest Feature):** Cruza los puntos proyectados con los fragmentos diminutos generados, asignando el identificador espacial definitivo.

- **Ruta B: Procesamiento de Líneas (Intersección por Grilla)**
  1.  **Proyección Base:** Transforma la topología a un sistema proyectado métrico (EPSG:3857).
  2.  **Grilla Dinámica:** Genera un tapiz de polígonos rectangulares (`st_make_grid`) cuyo tamaño es regulado por el `space.gap` y ajustado por la tolerancia angular.
  3.  **Corte Físico:** Ejecuta una intersección topológica estricta (`st_intersection`) entre las líneas de los vehículos y la grilla. Esto fractura las líneas largas exactamente en los bordes de cada celda.
  4.  **Ponderación:** Se calcula el largo residual de cada fragmento atrapado en la celda para utilizarlo como peso matemático.

### Fase 3: Agregación Topológica y Numérica

1. Mediante programación de datos ultra-rápida (`data.table`), los datos se agrupan por sus tres identificadores clave (entidad, fragmento de tiempo, fragmento de espacio).
2. Se aplica la función de compilación numérica configurada.
3. Se unifican o cosen los fragmentos geométricos topológicamente (`st_combine` $\to$ `st_line_merge` o herencia directa).
4. El objeto final es formateado, se descartan los artefactos nulos espaciales, se reproyecta a la norma estándar (WGS84 / EPSG:4326) y se retorna el cuerpo espacial completo.

## Ejemplos

### Ejemplo Rutas de Waze

```{r discretization.example, eval=FALSE}
line.to.polygon <- function(line, thickness = 2) {
  projected.line <- line %>% st_transform(crs = 3857)
  distance <- thickness / 2
  polygon <- projected.line %>% st_buffer(
    dist = distance,
    endCapStyle = "FLAT",
    joinStyle = "MITRE"
  ) %>% st_transform(crs = 4326)
...
degrees.format <- function(deg, sec.decimals = 2) {
  pos <- deg %>% abs()
...
filtered.waze.routes.lines.0 <- filtered.waze.routes %>%
  distinct(name, .keep_all = T) %>%
  create.lines()
filtered.waze.routes.lines <- filtered.waze.routes %>%
  create.lines()
filtered.waze.routes.polygons <- filtered.waze.routes %>%
  create.bbox.polygons()
sg <- 10
max.sg <- 200
tg <- 300
start.sys <- Sys.time()
discretization.example <- quantum.data(
  time.data = filtered.waze.routes.lines$updateTime,
  geom.data = filtered.waze.routes.lines$geometry,
  field.data = filtered.waze.routes.lines$speed,
  names.data = paste(filtered.waze.routes.lines$section.name, "-", filtered.waze.routes.lines$section.dir),
  space.gap = sg,
  time.gap = tg
) %>%
  arrange(time.bin) %>%
  mutate(
    time.bin = time.bin %>% as.POSIXct(tz = "UTC")
  )
end.sys <- Sys.time()
dt.sys <- end.sys - start.sys

discretization.example %>% saveRDS(file = "~/GitHub/data-sets-analysis/rds-files/discretization.example.rds")

#discretization.example <- readRDS(file = "~/GitHub/data-sets-analysis/rds-files/discretization.example.rds")
      ...
      column(
        ...
        wellPanel(
          sliderInput(
            ...
            min = discretization.example$time.bin %>% min(),
            max = discretization.example$time.bin %>% max(),
            value = discretization.example$time.bin %>% range(),
            step = tg,
            animate = T,
            width = "100%",
            timezone = "+0000"
...
base.grid.global.waze <- filtered.waze.routes.lines.0 %>%
  ungroup() %>%
  mutate(route.id = paste(section.name, "-", section.dir)) %>%
  group_by(route.id) %>%
  summarise(
    geometry = geometry %>% st_union() %>% st_line_merge(),
    .groups = "drop"
  ) %>%
  st_as_sf() %>%
  ...
  st_cast("LINESTRING", warn = F) %>%
  mutate(
    startX = purrr::map_dbl(geometry, ~st_coordinates(.x)[1, 1])
  ) %>%
  group_by(route.id) %>%
  mutate(
    sort.score = st_distance(geometry, geometry[which.min(startX)])[,1] %>% as.numeric()
  ) %>%
  arrange(route.id, sort.score) %>%
  mutate(
    physical.part = row_number(),
    base.id = paste(route.id, "Part", physical.part, sep = "_")
  ) %>%
  ungroup() %>%
  select(-startX, -sort.score) %>%
  st_transform(crs = 32718) %>%
  mutate(
    total.len = geometry %>% st_length() %>% as.numeric(),
    cuts = purrr::map(total.len, function(l){
      if(l <= sg) return(data.frame(s = 0, e = 1))
      dists <- seq(0, l, by = sg)
      if(dists[length(dists)] < l) dists <- c(dists, l)
      fracs <- dists / l
      data.frame(s = fracs[-length(fracs)], e = fracs[-1])
    })
  ) %>%
  tidyr::unnest(cuts) %>%
  mutate(
    geom = purrr::pmap(list(geometry, s, e), function(g, s, e) lwgeom::st_linesubstring(g, s, e)) %>% st_sfc(crs = 32718)
  ) %>%
  st_drop_geometry() %>%
  st_as_sf(sf_column_name = "geom") %>%
  mutate(
    len = geom %>% st_length() %>% as.numeric()
  ) %>%
  st_transform(crs = 4326) %>%
  rename(geometry = geom) %>%
  group_by(base.id) %>%
  mutate(
    piece.idx = row_number(),
    piece.id = paste(base.id, piece.idx, sep = "_")
  ) %>%
  ungroup() %>%
  select(-total.len, -s, -e, -physical.part)

discretization.example.sync <- discretization.example %>%
  mutate(centroid = geometry %>% st_centroid()) %>%
  split(.$name) %>%
  lapply(function(df) {
    route.name <- df$name[1] %>% as.character()
    sub.grid <- base.grid.global.waze %>% filter(route.id == route.name)
    if(sub.grid %>% nrow() > 0) {
      near.idx <- st_nearest_feature(df$centroid, sub.grid)
      df$piece.id <- sub.grid$piece.id[near.idx]
      return(df)
    }
    return(NULL)
  }) %>%
...
server <- function(input, output, options){
  pal <- colorNumeric(
    palette = c("#800", "#840", "#880", "#480", "#080", "#084", "#088", "#048", "#008", "#408", "#808"),
    domain = discretization.example$compiled.field
  )
  r.mapping <- reactive({
    input$space.agg %>% req()
    mapping <- base.grid.global.waze %>%
      st_drop_geometry() %>%
      select(piece.id, route.id, len) %>%
      mutate(
        part.idx = stringr::str_extract(piece.id, "(?<=_Part_)\\d+") %>% as.numeric(),
        piece.idx = stringr::str_extract(piece.id, "\\d+$") %>% as.numeric()
      ) %>%
      arrange(route.id, part.idx, piece.idx) %>%
      group_by(route.id) %>%
      mutate(
        cum.dist = len %>% cumsum() - (len / 2),
        agg.idx = cum.dist %/% input$space.agg,
        base.id = paste(route.id, "Part", part.idx, sep = "_"),
        new.space.bin = paste(route.id, agg.idx, sep = "_agg_")
      ) %>%
      ungroup() %>%
      select(piece.id, route.id, base.id, new.space.bin, agg.idx, cum.dist)
    return(mapping)
  })
  r.agg.grid <- reactive({
    mapping <- r.mapping()
    agg.grid <- base.grid.global.waze %>%
      inner_join(mapping, by = c("piece.id", "route.id", "base.id")) %>%
      arrange(route.id, cum.dist) %>%
      group_by(route.id, base.id, new.space.bin, agg.idx) %>%
      summarise(
        geometry = {
          coords <- (geometry %>% st_coordinates())[, 1:2]
          dups <- c(F, coords %>% diff() %>% abs() %>% rowSums() < 1e-6)
          list(coords[!dups, ] %>% st_linestring())
        },
        .groups = "drop"
      ) %>%
      mutate(geometry = geometry %>% st_sfc(crs = 4326)) %>%
      st_as_sf()
    return(agg.grid)
  })
  f.data <- reactive({
    input$time.range %>% req()
    grid <- r.agg.grid()
    mapping <- r.mapping()
    filtered.disc <- discretization.example.sync %>%
      st_drop_geometry() %>%
      filter(
        time.bin >= input$time.range[1] & 
        time.bin <= input$time.range[2]
      ) %>%
      inner_join(mapping, by = "piece.id") %>%
      group_by(new.space.bin) %>%
      summarise(
        field = compiled.field %>% weighted.mean(w = lenght %>% as.numeric(), na.rm = T),
        time.info = paste(time.bin %>% min() %>% format("%H:%M"), "-", time.bin %>% max() %>% format("%H:%M")),
        date.info = ifelse(
          time.bin %>% min() %>% format("%Y-%m-%d") == time.bin %>% max() %>% format("%Y-%m-%d"),
          time.bin %>% min() %>% format("%Y-%m-%d"),
          paste(time.bin %>% min() %>% format("%Y-%m-%d"), "-", time.bin %>% max() %>% format("%Y-%m-%d"))
        ),
        .groups = "drop"
      )
    active.routes <- filtered.disc$new.space.bin %>% stringr::str_replace("_agg_\\d+$", "") %>% unique()
    final.data <- grid %>%
      filter(route.id %in% active.routes) %>%
      left_join(filtered.disc, by = "new.space.bin") %>%
      group_by(route.id) %>%
      arrange(agg.idx, .by_group = T) %>%
      mutate(
        field = if(sum(!is.na(field)) > 1) {
          field %>% zoo::na.approx(na.rm = F, rule = 2)
        } else {
          field %>% mean(na.rm = T)
        },
        street.name = stringr::str_split_i(route.id, " - ", 1),
        section.dir = stringr::str_split_i(route.id, " - ", 2) 
      ) %>%
      tidyr::fill(time.info, date.info, .direction = "downup") %>%
      ungroup() %>%
      st_as_sf()
    return(final.data)
  })
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      setView(
        lng = ((filtered.waze.routes$minX + filtered.waze.routes$maxX) / 2) %>% mean(na.rm = T),
        lat = ((filtered.waze.routes$minY + filtered.waze.routes$maxY) / 2) %>% mean(na.rm = T),
        zoom = 12
      ) %>%
      addLayersControl(
        overlayGroups = filtered.waze.routes.polygons$section.dir %>% levels(),
  ...
  observe({
    ...
    updated.data <- f.data()
    if(updated.data %>% nrow() > 0){
      updated.polygons <- updated.data %>% 
        mutate(geometry = geometry %>% line.to.polygon(thickness = 7.5)) %>% 
        st_as_sf()
      updated.data %>% saveRDS(file = "~/GitHub/data-sets-analysis/rds-files/updated.data.rds")
      proxy %>%
        addPolylines(
          data = updated.data,
          color = ~field %>% pal(),
          group = ~section.dir,
          popup = ~paste(
            paste("Fecha:", date.info),
            paste("Hora:", time.info),
            paste("Velocidad:", (field * 18 / 5) %>% round(2), "km/h"),
            paste("Largo:", geometry %>% st_transform(crs = 32718) %>% st_length() %>% as.numeric() %>% round(2), "m"),
            sep = "<br>"
          ),
          opacity = 1
        ) %>%
        addPolygons(
          data = updated.polygons,
          group = ~section.dir,
          fill = F,
          color = "black",
          weight = 1.5
...
```{r discretization.example, eval=FALSE}
readRDS(file = "~/GitHub/data-sets-analysis/rds-files/updated.data.rds") %>%
  mutate(
    len = geometry %>% st_length()
  ) %>%
  View()
```
...
```{r discretization.example.1 eval=FALSE}
gps.streets <- get.street(
  lng.data = gps.data.df$Longitud,
  lat.data = gps.data.df$Latitud
)

gps.streets %>% saveRDS(file = "~/GitHub/data-sets-analysis/rds-files/gps.streets.rds")

#gps.streets <- readRDS(file = "~/GitHub/data-sets-analysis/rds-files/gps.streets.rds")

is.in.carrera <- gps.streets$street.name %in% c("Avenida Los Carrera", "Avenida Los Carrera Poniente")
filtered.gps.data <- gps.data.df[is.in.carrera,]
filtered.gps.streets <- gps.streets[is.in.carrera,]
```
...
discretization.example.1 <- quantum.data(
  time.data = filtered.gps.data$Fecha,
  lng.data = filtered.gps.data$Longitud,
  lat.data = filtered.gps.data$Latitud,
  field.data = filtered.gps.data$`Velocidad Km/h`,
  names.data = filtered.gps.data$Vehiculo,
  time.gap = tg.1,
  space.gap = sg.1
)

discretization.example.1 %>% saveRDS(file = "~/GitHub/data-sets-analysis/rds-files/discretization.example.1.rds")

#discretization.example.1 <- readRDS(file = "~/GitHub/data-sets-analysis/rds-files/discretization.example.1.rds")

ui.1 <- fluidPage(
  titlePanel("Mapa con Filtro de Tiempo y Vehículos"),
  sidebarLayout(
    fluidRow(
      column(
        width = 12,
        wellPanel(
          sliderInput(
            "time.range",
            "Selecciona el rango de tiempo:",
            min = discretization.example.1$time.bin %>% min(),
            max = discretization.example.1$time.bin %>% max(),
            value = discretization.example.1$time.bin %>% range(),
            step = tg.1,
            animate = T,
            width = "100%"
          ),
          checkboxGroupInput(
            "selected.vehicles",
            "Selecciona los vehículos a promediar:",
            choices = discretization.example.1$name %>% levels(),
            selected = discretization.example.1$name %>% levels(),
            inline = T
          ),
          sliderInput(
            "space.agg",
...
base.grid.global <- filtered.gps.streets %>%
  ungroup() %>%
  st_drop_geometry() %>%
  st_as_sf(sf_column_name = "street.geom") %>%
  rename(geometry = street.geom) %>%
  distinct(street.name, lane.id, .keep_all = T) %>%
  select(street.name, lane.id) %>%
  st_collection_extract("LINE") %>%
  st_cast("MULTILINESTRING", warn = F) %>%
  ...
  st_transform(crs = 3857) %>%
  mutate(
    len = geometry %>% st_length() %>% as.numeric(),
    cuts = purrr::map(len, function(l){
      if(l <= sg.1) return(data.frame(s = 0, e = 1))
      dists <- seq(0, l, by = sg.1)
      if(dists[dists %>% length()] < l) dists <- c(dists, l)
      fracs <- dists / l
      data.frame(s = fracs[-length(fracs)], e = fracs[-1])
    })
  ...
  tidyr::unnest(cuts) %>%
  mutate(
    geom = purrr::pmap(list(geometry, s, e), function(g, s, e) lwgeom::st_linesubstring(g, s, e)) %>% st_sfc(crs = 3857)
  ) %>%
  st_drop_geometry() %>%
  st_as_sf(sf_column_name = "geom") %>%
  rename(geometry = geom) %>%
...
server.1 <- function(input, output, options){
  pal <- colorNumeric(
    palette = c("#800", "#880", "#080", "#088", "#008", "#808"),
    domain = discretization.example.1$compiled.field
  )
  f.points.centered <- reactive({
    input$time.range %>% req()
    (input$selected.vehicles %>% length() > 0) %>% req()
    filtered.gps.streets %>% mutate(
      Fecha = filtered.gps.data$Fecha,
      Vehiculo = filtered.gps.data$Vehiculo
    ) %>% filter(
      Fecha >= input$time.range[1] &
      Fecha <= input$time.range[2] &
      Vehiculo %in% input$selected.vehicles
    )
  })
  f.points <- reactive({
    input$time.range %>% req()
    (input$selected.vehicles %>% length() > 0) %>% req()
    filtered.gps.data %>% filter(
      Fecha >= input$time.range[1] &
      Fecha <= input$time.range[2] &
      Vehiculo %in% input$selected.vehicles
    )
  ...
  f.data <- reactive({
    ...
    (input$selected.vehicles %>% length() > 0) %>% req()
    input$space.agg %>% req()
    agg.factor <- c(1, (input$space.agg / sg.1) %>% round()) %>% max()
    agg.grid <- base.grid.global %>%
      mutate(
        piece.idx = stringr::str_extract(piece.id, "\\d+$") %>% as.numeric(),
        stable.id = stringr::str_replace(piece.id, "_\\d+_", "_"),
        base.id = stringr::str_replace(stable.id, "_\\d+$", ""),
        agg.idx = (piece.idx - 1) %/% agg.factor,
        new.space.bin = paste(base.id, agg.idx, sep = "_agg_")
      ) %>%
      group_by(base.id, new.space.bin, agg.idx) %>%
      summarise(
        geometry = geometry %>% st_combine() %>% st_line_merge(),
        .groups = "drop"
      ) %>%
      st_as_sf()
    filtered.disc <- discretization.example.1 %>%
      st_drop_geometry() %>%
      filter(
        time.bin >= input$time.range[1] & 
        time.bin <= input$time.range[2] &
        name %>% as.character() %in% input$selected.vehicles
      ) %>%
      mutate(
        space.bin.str = space.bin %>% as.character(),
        piece.idx = stringr::str_extract(space.bin.str, "\\d+$") %>% as.numeric(),
        stable.id = stringr::str_replace(space.bin.str, "_\\d+_", "_"),
        base.id = stringr::str_replace(stable.id, "_\\d+$", ""),
        agg.idx = (piece.idx - 1) %/% agg.factor,
        new.space.bin = paste(base.id, agg.idx, sep = "_agg_")
      ) %>%
      ...
      summarise(
        field = compiled.field %>% weighted.mean(w = lenght %>% as.numeric(), na.rm = T),
        vehicles.involved = paste(name %>% unique(), collapse = ", "),
        time.info = paste(time.bin %>% min() %>% format("%H:%M"), "-", time.bin %>% max() %>% format("%H:%M")),
        date.info =  ifelse(
          time.bin %>% min() %>% format("%Y-%m-%d") == time.bin %>% max() %>% format("%Y-%m-%d"),
          time.bin %>% min() %>% format("%Y-%m-%d"),
          paste(time.bin %>% min() %>% format("%Y-%m-%d"), "-", time.bin %>% max() %>% format("%Y-%m-%d"))),
        street.name = new.space.bin %>% stringr::str_split_i("_", 1),
        .groups = "drop"
      )
    active.bases <- filtered.disc$new.space.bin %>% stringr::str_replace("_agg_\\d+$", "") %>% unique()
    final.data <- agg.grid %>%
      filter(base.id %in% active.bases) %>%
      left_join(filtered.disc, by = "new.space.bin") %>%
      group_by(base.id) %>%
      arrange(agg.idx, .by_group = T) %>%
      mutate(
        field = zoo::na.approx(field, na.rm = F, rule = 2),
        street.name = base.id %>% stringr::str_split_i("_", 1)
      ) %>%
      tidyr::fill(vehicles.involved, time.info, date.info, .direction = "downup") %>%
      ungroup() %>%
      st_as_sf()
    return(final.data)
  })
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      addScaleBar(position = "bottomleft") %>%
      setView(
        lng = gps.data.df$Longitud %>% mean(na.rm = T),
        lat = gps.data.df$Latitud %>% mean(na.rm = T),
        zoom = 12
      ) %>% addLayersControl(
        overlayGroups = c("Calles", "Celdas", "Puntos Originales", "Puntos Centrados")
      )
  })
  observe({
    proxy <- leafletProxy("map") %>% clearShapes()
    updated.data <- f.data()
    filtered.gps.data.df <- f.points()
    centered.points <- f.points.centered()
    if(filtered.gps.data.df %>% nrow() > 0){
        proxy %>%
          addCircles(
            data = filtered.gps.data.df,
            group = "Puntos Originales",
            color = "red", opacity = 1, lng = ~Longitud, lat = ~Latitud, radius = 1
          ) %>% 
          addCircles(
            data = centered.points,
            group = "Puntos Centrados",
            color = "blue", opacity = 1, lng = ~lng.street, lat = ~lat.street, radius = 1
          )
    }
    if(updated.data %>% nrow() > 0){
      updated.polygons <- updated.data %>% 
      ...
      proxy %>%
        addPolylines(
          ...
          group = "Calles",
          popup = ~paste(
            paste("Vehículos:", vehicles.involved),
            paste("Calle:", street.name),
            paste("Largo:", geometry %>% st_transform(crs = 3857) %>% st_length() %>% as.numeric() %>% round(2), "m"),
            paste("Fecha:", date.info),
            paste("Hora:", time.info),
            paste("Velocidad:", field %>% round(2), "km/h"),
            sep = "<br>"
          ),
          opacity = 1
        ) %>%
...
CREATE TABLE IF NOT EXISTS route.name (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name VARCHAR(255)
)
...
CREATE TABLE IF NOT EXISTS waze_route (
  ...
  name_id INTEGER REFERENCES route.name(id),
