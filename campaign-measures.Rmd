...
key.objects.campaign <- c("meters.per.degree.lat", "meters.per.degree.lng", "waze.routes", "waze.routes.df", "climate.data.df", "gps.data.df", "speed.pistols.df", "police.type.accident", "police.type.vehicle")
...
gps.data.df <- gps.data.df %>% select(-Hora)
...
differences <- gps.data.df %>%
  mutate(
    prev.time = Fecha %>% lag(),
    same.vh = (Vehiculo == (Vehiculo %>% lag(default = Vehiculo %>% first() - 1)))
  ) %>%
  mutate(
    diff = if_else(same.vh, (Fecha - prev.time) %>% as.numeric(), NA_real_)
  ) %>%
  pull(diff)
ggplot() +
...
distances <- gps.data.df %>%
  ...
  mutate(
    dist = if_else(same.vh, haversine.dist(Latitud, Longitud, prev.y, prev.x), NA_real_)
  ) %>%
  pull(dist)
ggplot() +
